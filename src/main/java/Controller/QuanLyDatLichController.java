...
// Phương thức tính tổng tiền hóa đơn - ĐÃ SỬA để trả về BigDecimal
    ...
    private void inHoaDonPDF(DatLich datLich) {
        try {
            ...
            inHoaDonPDFDetail(datLich, khachHang, giuong, tongTien, Auth.tenDangNhap);
...
// Sửa đổi phương thức inHoaDonPDFDetail - Thêm hiển thị điểm tích lũy
  // Sửa đổi phương thức inHoaDonPDFDetail - Thêm hiển thị điểm tích lũy và đường dẫn động
public void inHoaDonPDFDetail(DatLich datLich, KhachHang khachHang, Giuong giuong, double tongTien, String tenNV) {
    ...
    try {
        ...
        String billDirectory = projectRoot + File.separator + "src" + File.separator + "main" + 
                              File.separator + "resources" + File.separator + "bill";
        ...
        if (tongCong > 0) {
            try {
                ...
                // Lưu QR code tạm vào thư mục bill
